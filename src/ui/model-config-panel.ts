...
export const MODEL_CONFIG_PANEL_MIN_WIDTH = 820;
...
export function buildModelConfigPanelHtml(
  config: ModelsConfig,
  ...
  const safeProviderOrder = JSON.stringify(providerOrder).replaceAll("<", "\\u003c");
  return `<!doctype html>
...
.workspace{grid-template-columns:max-content max-content minmax(320px,1fr);min-width:${MODEL_CONFIG_PANEL_MIN_WIDTH}px}.item{white-space:nowrap;overflow:visible;text-overflow:clip}.input-types{display:flex;gap:12px}.input-types label{display:flex;align-items:center;gap:4px;margin:0}
...
<div class="field"><label data-i18n="input"></label><div class="input-types"><label><input id="model-input-text" type="checkbox" value="text"> text</label><label><input id="model-input-image" type="checkbox" value="image"> image</label></div></div>
...
var lang="zh",zoom=1,config=${safeConfig},providerIndex=0,modelIndex=0,activeCurrency="CNY",pendingAction=null,renderedApiKey="",providerOrder=${safeProviderOrder};
...
function el(id){return document.getElementById(id)}function t(key){return UI_TEXT[lang][key]||key}function texts(){document.querySelectorAll("[data-i18n]").forEach(function(node){node.textContent=t(node.dataset.i18n)});el("gd-lang").textContent=lang==="zh"?"EN":"中文"}function syncProviderOrder(){var providers=config.providers||{};providerOrder=providerOrder.filter(function(id,index){return typeof id==="string"&&Object.prototype.hasOwnProperty.call(providers,id)&&providerOrder.indexOf(id)===index});Object.keys(providers).forEach(function(id){if(providerOrder.indexOf(id)===-1)providerOrder.push(id)})}function providerEntries(){syncProviderOrder();return providerOrder.map(function(id){return [id,config.providers[id]]})}function selectedProvider(){return providerEntries()[providerIndex]||null}function selectedModel(){var p=selectedProvider();return p&&Array.isArray(p[1].models)?p[1].models[modelIndex]||null:null}function uniqueId(prefix,items){var n=1;while(items.some(function(id){return id===prefix+"-"+n}))n++;return prefix+"-"+n}function moveProvider(offset){saveForm();var entries=providerEntries(),next=providerIndex+offset;if(next<0||next>=entries.length)return;var item=entries.splice(providerIndex,1)[0];entries.splice(next,0,item);providerOrder=entries.map(function(entry){return entry[0]});config.providers=Object.fromEntries(entries);syncProviderOrder();providerIndex=next;render()}function addProvider(){saveForm();var id=uniqueId("provider",Object.keys(config.providers));config.providers[id]={models:[]};providerOrder.push(id);syncProviderOrder();providerIndex=providerEntries().length-1;modelIndex=0;render()}function addModel(){var entry=selectedProvider();if(!entry)return;saveForm();var models=entry[1].models||[];var id=uniqueId("model",models.map(function(model){return model.id}));entry[1].models=models.concat([{id:id,contextWindow:300000,maxTokens:24000}]);modelIndex=models.length;render()}function deleteSelected(){if(pendingAction==="delete-provider"){var entries=providerEntries();if(entries.length){var removed=entries.splice(providerIndex,1)[0];providerOrder=providerOrder.filter(function(id){return id!==removed[0]});config.providers=Object.fromEntries(entries);syncProviderOrder();providerIndex=Math.min(providerIndex,Math.max(0,entries.length-1));modelIndex=0;render()}}else if(pendingAction==="delete-model"){var entry=selectedProvider();if(entry&&Array.isArray(entry[1].models)){entry[1].models.splice(modelIndex,1);modelIndex=Math.min(modelIndex,Math.max(0,entry[1].models.length-1));render()}}pendingAction=null}function requestDelete(action){if(action==="delete-provider"){var provider=selectedProvider();if(!provider)return;var models=provider[1].models;if(Array.isArray(models)&&models.length){status("请先清空供应商中的模型",false);return}}if(action==="delete-model"&&!selectedModel())return;pendingAction=action;showConfirmation(action)}
function renderLists(){var providers=providerEntries(),pl=el("provider-list"),ml=el("model-list");pl.textContent="";providers.forEach(function(entry,i){var row=document.createElement("div");row.className="item"+(i===providerIndex?" active":"");row.tabIndex=i===providerIndex?0:-1;row.role="option";row.draggable=true;row.textContent=entry[0];row.addEventListener("click",function(){saveForm();providerIndex=i;modelIndex=0;render()});row.addEventListener("dragstart",function(e){e.dataTransfer.setData("text/plain",String(i))});row.addEventListener("dragover",function(e){e.preventDefault()});row.addEventListener("drop",function(e){e.preventDefault();saveForm();var from=Number(e.dataTransfer.getData("text/plain")),entries=providerEntries(),item=entries.splice(from,1)[0];entries.splice(i,0,item);providerOrder=entries.map(function(entry){return entry[0]});config.providers=Object.fromEntries(entries);syncProviderOrder();providerIndex=i;modelIndex=0;render()});pl.appendChild(row)});ml.textContent="";var p=selectedProvider(),models=p&&Array.isArray(p[1].models)?p[1].models:[];models.forEach(function(model,i){var row=document.createElement("div");row.className="item"+(i===modelIndex?" active":"");row.tabIndex=i===modelIndex?0:-1;row.role="option";row.textContent=model.name||model.id;row.addEventListener("click",function(){saveForm();modelIndex=i;render()});ml.appendChild(row)})}
function json(value){return JSON.stringify(value===undefined?{}:value,null,2)}function costValue(value){var number=typeof value==="number"?value:0;return activeCurrency==="CNY"?number:number/7}
function renderForm(){var p=selectedProvider(),m=selectedModel(),provider=p?p[1]:{},model=m||{};el("model-view").classList.toggle("hidden",!m);el("provider-id").value=p?p[0]:"";var api=provider.api||"openai-completions",apiOption=el("provider-api").querySelector('option[value="'+api+'"]');if(!apiOption){apiOption=document.createElement("option");apiOption.value=api;apiOption.textContent=api;el("provider-api").appendChild(apiOption)}el("provider-api").value=api;el("provider-base-url").value=provider.baseUrl||"";renderedApiKey=provider.apiKey||"";el("provider-api-key").value=renderedApiKey;el("provider-api-key").type=renderedApiKey==="[REDACTED]"?"password":"text";el("provider-headers").value=json(provider.headers);el("provider-auth-header").checked=provider.authHeader===true;el("model-id").value=model.id||"";el("model-name").value=model.name||"";el("model-reasoning").checked=model.reasoning!==false;el("model-context-window").value=model.contextWindow||"";el("model-max-tokens").value=model.maxTokens||"";var inputTypes=Array.isArray(model.input)?model.input:[];["text","image"].forEach(function(type){el("model-input-"+type).checked=!Array.isArray(model.input)||inputTypes.indexOf(type)!==-1});var thinkingMap=model.thinkingLevelMap||{};["medium","high","xhigh"].forEach(function(level){var value=thinkingMap[level];el("thinking-"+level).checked=value===undefined?level!=="xhigh":typeof value==="string"&&value.length>0});el("currency").value=activeCurrency;["input","output","cacheRead","cacheWrite"].forEach(function(key){el("cost-"+(key==="cacheRead"?"cache-read":key==="cacheWrite"?"cache-write":key)).value=costValue(model.cost&&model.cost[key])})}
function optional(target,key,value){if(value===""||value===undefined)delete target[key];else target[key]=value}function parsed(id){return JSON.parse(el(id).value||"{}")}function readCost(id){var value=Number(el(id).value);if(!Number.isFinite(value)||value<0)throw new Error("Invalid cost: "+id);return activeCurrency==="CNY"?value:value*7}
function saveForm(){var entry=selectedProvider();if(!entry)return;var oldId=entry[0],p=entry[1],newId=el("provider-id").value.trim()||oldId;optional(p,"api",el("provider-api").value.trim()||"openai-completions");optional(p,"baseUrl",el("provider-base-url").value.trim());var apiKey=el("provider-api-key").value;if(apiKey!==renderedApiKey)optional(p,"apiKey",apiKey);p.headers=parsed("provider-headers");p.authHeader=el("provider-auth-header").checked;var m=selectedModel();if(m){m.id=el("model-id").value.trim();optional(m,"name",el("model-name").value.trim());m.reasoning=el("model-reasoning").checked;optional(m,"contextWindow",el("model-context-window").value?Number(el("model-context-window").value):"");optional(m,"maxTokens",el("model-max-tokens").value?Number(el("model-max-tokens").value):"");var inputs=["text","image"].filter(function(type){return el("model-input-"+type).checked});m.input=inputs;var thinking=Object.assign({medium:null,high:null,off:null,minimal:null,low:null,xhigh:null,max:null},m.thinkingLevelMap);Object.keys(thinking).forEach(function(level){thinking[level]=null});["medium","high","xhigh"].forEach(function(level){thinking[level]=el("thinking-"+level).checked?(typeof thinking[level]==="string"&&thinking[level]?thinking[level]:level):null});m.thinkingLevelMap=thinking;m.cost=Object.assign({},m.cost,{input:readCost("cost-input"),output:readCost("cost-output"),cacheRead:readCost("cost-cache-read"),cacheWrite:readCost("cost-cache-write")})}if(newId!==oldId){var next={};Object.entries(config.providers).forEach(function(item){next[item[0]===oldId?newId:item[0]]=item[1]});config.providers=next;providerOrder=providerOrder.map(function(id){return id===oldId?newId:id});syncProviderOrder()}}
function render(){renderLists();renderForm();var providers=providerEntries(),providerCount=providers.length,modelCount=selectedProvider()&&Array.isArray(selectedProvider()[1].models)?selectedProvider()[1].models.length:0;el("provider-up").disabled=providerIndex<=0;el("provider-down").disabled=providerIndex>=providerCount-1;el("provider-delete").disabled=providerCount===0;el("model-add").disabled=providerCount===0;el("model-delete").disabled=modelCount===0;texts()}function status(message,ok){var node=el("status");node.textContent=message||"";node.className="status"+(message?" "+(ok?"success":"error"):"")}function send(action){try{if(action!=="cancel"){saveForm();syncProviderOrder()}window.glimpse.send({action:action,config:config,providerOrder:providerOrder.slice()})}catch(error){status(String(error),false)}}

...
<section class="pane"><div class="tabs"><button class="tool tab active" id="tab-model" data-i18n="modelConfig"></button><button class="tool tab" id="tab-json" data-i18n="fullJson"></button></div>
...
</div></div><div id="json-view" class="hidden"><textarea class="advanced" id="advanced-json"></textarea></div></section></main>
...
var UI_TEXT={zh:{title:"模型配置",providers:"供应商",models:"模型",providerConfig:"供应商配置",modelConfig:"模型配置",fullJson:"完整 JSON",providerId:"供应商 ID",api:"API 类型",baseUrl:"基础 URL",apiKey:"API Key（密钥）",headers:"Headers（请求头 JSON）",authHeader:"使用认证请求头",modelId:"模型 ID",modelName:"模型名称",reasoning:"推理模型",contextWindow:"上下文窗口",maxTokens:"最大输出",input:"输入类型",costInput:"输入成本 / 1M",costOutput:"输出成本 / 1M",costCacheRead:"缓存读取成本 / 1M",costCacheWrite:"缓存写入成本 / 1M",currency:"币种",modelAdvanced:"模型高级字段 JSON",import:"导入 YAML",validate:"校验",preview:"预览差异",cancel:"取消 Esc",apply:"应用",confirm:"确定",valid:"配置有效",invalid:"配置无效"},en:{title:"Model Config",providers:"Providers",models:"Models",providerConfig:"Provider config",modelConfig:"Model config",fullJson:"Full JSON",providerId:"Provider ID",api:"API type",baseUrl:"Base URL",apiKey:"API key",headers:"Headers JSON",authHeader:"Use auth header",modelId:"Model ID",modelName:"Model name",reasoning:"Reasoning model",contextWindow:"Context window",maxTokens:"Max output",input:"Input types",costInput:"Input cost / 1M",costOutput:"Output cost / 1M",costCacheRead:"Cache read cost / 1M",costCacheWrite:"Cache write cost / 1M",currency:"Currency",modelAdvanced:"Model advanced JSON",import:"Import YAML",validate:"Validate",preview:"Preview diff",cancel:"Cancel Esc",apply:"Apply",confirm:"OK",valid:"Config is valid",invalid:"Config is invalid"}};
...
function useAdvanced(){config=JSON.parse(el("advanced-json").value);providerIndex=Math.min(providerIndex,Math.max(0,providerEntries().length-1));modelIndex=0;render()}function render(){renderLists();renderForm();texts()}function send(action){try{if(action!=="import"&&action!=="cancel"){if(!el("json-view").classList.contains("hidden"))useAdvanced();else saveForm()}window.glimpse.send({action:action,config:config})}catch(error){status(String(error),false)}}function status(message,ok){var node=el("status");node.textContent=message;node.className="status "+(ok?"success":"error")}
["import","validate","preview","apply","confirm","cancel"].forEach(function(action){el("b-"+action).addEventListener("click",function(){send(action)})});el("advanced-json").addEventListener("change",function(){try{useAdvanced();status(t("valid"),true)}catch(error){status(String(error),false)}});el("tab-model").addEventListener("click",function(){el("model-view").classList.remove("hidden");el("json-view").classList.add("hidden");el("tab-model").classList.add("active");el("tab-json").classList.remove("active")});el("tab-json").addEventListener("click",function(){saveForm();el("model-view").classList.add("hidden");el("json-view").classList.remove("hidden");el("tab-model").classList.remove("active");el("tab-json").classList.add("active")});el("currency").addEventListener("change",function(){saveForm();activeCurrency=el("currency").value;renderForm()});el("gd-lang").addEventListener("click",function(){lang=lang==="zh"?"en":"zh";texts()});function setZoom(value){zoom=Math.max(.8,Math.min(1.5,Math.round(value*10)/10));document.body.style.zoom=String(zoom)}el("gd-zoom-out").addEventListener("click",function(){setZoom(zoom-.1)});el("gd-zoom-in").addEventListener("click",function(){setZoom(zoom+.1)});el("gd-zoom-reset").addEventListener("click",function(){setZoom(1)});

...
var lang="zh",zoom=1,config=${safeConfig},providerIndex=0,modelIndex=0,activeCurrency="CNY",pendingAction=null;
...
function render(){renderLists();renderForm();texts()}function status(message,ok){var node=el("status");node.textContent=message||"";node.className="status"+(message?" "+(ok?"success":"error"):"")}function send(action){try{if(action!=="cancel")saveForm();window.glimpse.send({action:action,config:config})}catch(error){status(String(error),false)}}
var CONFIRM_TEXT={zh:{cancel:{title:"取消模型配置？",message:"未应用的修改将丢失，确定关闭窗口？",accept:"放弃修改"},apply:{title:"应用模型配置？",message:"保存当前修改并保持窗口打开。",accept:"应用"},confirm:{title:"保存并关闭模型配置？",message:"保存当前修改并关闭窗口。",accept:"确定"}},en:{cancel:{title:"Cancel model configuration?",message:"Unapplied changes will be lost. Close this window?",accept:"Discard changes"},apply:{title:"Apply model configuration?",message:"Save the current changes and keep this window open.",accept:"Apply"},confirm:{title:"Save and close model configuration?",message:"Save the current changes and close this window.",accept:"OK"}}};
...
["validate","preview","apply","confirm","cancel"].forEach(function(action){el("b-"+action).addEventListener("click",function(){requestAction(action)})});el("confirm-reject").addEventListener("click",dismissConfirmation);el("confirm-accept").addEventListener("click",function(){var action=pendingAction;dismissConfirmation();if(action)send(action)});el("confirm-dialog").addEventListener("cancel",function(event){event.preventDefault();dismissConfirmation()});el("currency").addEventListener("change",function(){saveForm();activeCurrency=el("currency").value;renderForm()});el("gd-lang").addEventListener("click",function(){lang=lang==="zh"?"en":"zh";texts()});function setZoom(value){zoom=Math.max(.8,Math.min(1.5,Math.round(value*10)/10));document.body.style.zoom=String(zoom)}el("gd-zoom-out").addEventListener("click",function(){setZoom(zoom-.1)});el("gd-zoom-in").addEventListener("click",function(){setZoom(zoom+.1)});el("gd-zoom-reset").addEventListener("click",function(){setZoom(1)});

...
- 默认不创建 Git 分支，直接在当前分支工作；只有用户明确要求分支、PR 或协作流程时才创建分支。
- 如果用户明确要求创建分支，使用 `feat/*`、`fix/*`、`refactor/*`、`chore/*`、`docs/*` 或 `test/*` 等清晰命名。
- 当前在 `main/master` 时，默认仍按用户要求直接工作；保护分支、禁止强推和禁止改写历史的安全红线不变。
